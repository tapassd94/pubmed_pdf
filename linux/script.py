...
class script:
    ...
    def get_pmcid_from_pmid(ids):
        ...
        for id in ids:
            ...
            if "pmcid" in xml["records"][0].keys():
                ...
                pmcid_pmid[xml["records"][0]["pmcid"]]["DOI"] = xml["records"][0]["doi"]
                bar.next()
    ...
    def get_abstract_from_pmcid(ids):
        ...
        for pmcid in ids.keys():
            ...
            ids[pmcid]["citations"] = dict(cit)

            bar.next()

        bar.finish()
        print(
            "Added article title, author names, date of journal and abstract to all PMCIDs"
        )
        return pmcid_pmid
    ...
    def save_pdf(ids, path, savePdf=False, fname="./", max_result=1):
        ...
        timer = 5
        print()
        bar = Bar("Saving PDF's + extracting text", max=max_result)
        for pmc in ids.keys():
            if count > max_result:
                break
